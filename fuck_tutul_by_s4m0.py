# python3
# CHANNEL : @
# Github: https://github.com/Sam86008
# THIS TOOL FUCKED BY SAMO
# DECRYPTED BY SAMO CRACKER
...
def obfuscate(VARIABLE_NAME, file_content):
    ...
    for _ in range(int(len(b64_content) / OFFSET) + 1):
        ...
        index += OFFSET
    code += f'exec(__import__("\\x62\\x61\\x73\\x65\\x36\\x34").b64decode({VARIABLE_NAME}.encode("Tutul_King/HACKER/Tutul_King/HACKER")).decode("Tutul_King/HACKER/Tutul_King/HACKER"))'
    return code
...
def encryptsh():
    ...
    with open(".temp",'r') as temp_f, open(out_file,'w') as out_f:
        filedata = temp_f.read()
        out_f.write("# Encrypted by : Tutul King\nBUT DECRYPTED AND FUCKED BY @I4M_S4M0 \n# Whatsapp OF FUCKER : +8801608843956\n# CHANNEL : https://t.me/tools_free_termux/\n\n"+filedata)
    os.remove(".temp")
    sprint(success + out_file + " saved in "+pwd)
    mover(out_file)
